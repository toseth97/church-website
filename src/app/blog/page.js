...
async function getEvents() {
    try {
        const res = await fetch(
            `${process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000"}/api/events?limit=20`,
            {
                cache: "no-store",
            },
        );
                    ...
                    {events.length > 0 ? (
                            ...
                            {events.slice(0, 4).map((event, i) => (
                                <Reveal key={event._id} delayMs={80 + i * 60}>
                                    <div className="group bg-gradient-to-br from-[#FFF8F0] to-[#FFF1E2] p-8 rounded-xl card-hover border border-[#F2C79B]/10">
                                        ...
                                        <h3 className="font-bold text-lg mb-4 group-hover:text-[#c9a27e] transition-colors">
                                            {event.title}
                                        </h3>
                        ...
                        <div className="grid md:grid-cols-4 gap-8">
                            {[1, 2, 3, 4].map((item) => (
                                <div
                                    key={item}
                                    className="bg-[#fbf4ee] p-8 rounded-xl"
                                >
                                    <p className="uppercase text-xs font-bold tracking-[0.2em] text-[#c9a27e] mb-3">
                                        Coming Soon
                                    </p>
                                    ...
                                    <p className="text-sm text-gray-600">
                                        New events and blog posts are coming
                                        soon.
                                    </p>
